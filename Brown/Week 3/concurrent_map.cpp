...
public:
    ...
    WriteAccess operator[](const K& key) {
        const size_t bucket_idx = hasher(key) % buckets_count;
        return { lock_guard<mutex>(mutexes[bucket_idx]), buckets_[bucket_idx][key] };
    }

    ReadAccess At(const K& key) const {
        const size_t bucket_idx = hasher(key) % buckets_count;
        return { lock_guard<mutex>(mutexes[bucket_idx]), buckets_[bucket_idx].at(key) };
    }

    bool Has(const K& key) const {
        const size_t bucket_idx = hasher(key) % buckets_count;
        return buckets_[bucket_idx].count(key) > 0;
    }
