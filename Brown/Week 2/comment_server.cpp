...
string FormFirstLine(HttpCode code) {
    string result = "HTTP/1.1 " + to_string(static_cast<int>(code));
    if (code == HttpCode::Ok) {
        result += " OK\n";
    } 
...
public:
    ...
    friend ostream& operator << (ostream& output, const HttpResponse& resp) {
        output << FormFirstLine(resp.code_);
    ...
    ParsedResponse GetInfo() const {
        return { static_cast<int>(code_), headers, content_ };
    }
...
void Test(CommentServer& srv, const HttpRequest& request, const ParsedResponse& expected) {
    ...
    ASSERT_EQUAL(resp.GetInfo().code, expected.code);
    ASSERT_EQUAL(resp.GetInfo().headers, expected.headers);
    ASSERT_EQUAL(resp.GetInfo().content, expected.content);
}
