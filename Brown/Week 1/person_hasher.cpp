...
struct AddressHasher {
    size_t operator()(const Address& address) const {
        ...
        return shasher(address.city) * x * x
             + shasher(address.street) * x
...
struct PersonHasher {
    size_t operator()(const Person& person) const {
        const size_t x = 947;
        return address_hasher.shasher(person.name) * x * x * x
             + address_hasher.ihasher(person.height) * x * x
             + dhasher(person.weight) * x 
